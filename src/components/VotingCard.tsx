...
interface VotingCardProps {
  ...
  description: string;
  icon: string;
  onVote: (category: VoteCategory, candidate: string) => string | null;
...
export const VotingCard = ({
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (candidate.trim()) {
      const code = onVote(category, candidate.trim());
      if (code) {
        setVoteCode(code);
        setVotedCandidate(candidate.trim());
        setShowConfirmation(true);
        toast({
          title: "Voto registrado!",
          description: "Agora confirme seu voto via WhatsApp.",
        });
        setCandidate('');
      }
    }
